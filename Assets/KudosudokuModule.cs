...
#pragma warning restore 414
    ...
    public IEnumerator ProcessTwitchCommand(string command)
    {
        ...
        else if (_activeSquare != null && _codings[_activeSquare.Value] == Coding.Letters && (m = Regex.Match(command, @"^\s*(?:(?:letters?|submit|enter|input)\s+)?([A-Z])\s*$", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)).Success)
        {
            ...
            {
                var current = _curLetter - 'A';
                var distance = (Math.Abs(current - target) + 13) % 26 - 13;
                if (current > target)
                    distance *= -1;
                if (distance > 4)
                    yield return new[] { LetterUpDownButtons[0] };
                else if (distance > 0)
                    yield return new[] { LetterUpDownButtons[1] };
                else if (distance < -4)
                    yield return new[] { LetterUpDownButtons[3] };
                else if (distance < 0)
                    yield return new[] { LetterUpDownButtons[2] };
                yield return "trycancel";
                Debug.LogFormat("<Kudosudoku #{0}> Letter distance: {1}", _moduleId, distance);
            }
            yield return new[] { Squares[_activeSquare.Value] };
        }
    ...
    public IEnumerator TwitchHandleForcedSolve()
    {
        ...
        foreach (var sq in undoneSqs)
        {
            ...
            switch (_codings[sq])
            {
                ...
                case Coding.Letters:
                    var target = _numberNames[_solution[sq]] - 'A';
                    var current = _curLetter - 'A';
                    var distance = (Math.Abs(current - target) + 13) % 26 - 13;
                    while (distance != 0)
                    {
                        current = _curLetter - 'A';
                        distance = (Math.Abs(current - target) + 13) % 26 - 13;
                        if (current > target)
                            distance *= -1;
                        if (distance > 4)
                            LetterUpDownButtons[0].OnInteract();
                        else if (distance > 0)
                            LetterUpDownButtons[1].OnInteract();
                        else if (distance < -4)
                            LetterUpDownButtons[3].OnInteract();
                        else if (distance < 0)
                            LetterUpDownButtons[2].OnInteract();
